    ...
    if( opt.mode == MODE_PRIVATE )
    {
        ...
        if( mbedtls_pk_get_type( &key ) == MBEDTLS_PK_ECKEY )
        {
            ...
            if( opt.output_format == OUTPUT_FORMAT_BIN )
            {
                ...
                {
                    mbedtls_printf( " failed\n  !  not enough memory for X coordinate - %d bytes left but %ld bytes needed \n\n", (unsigned int) remaining_bytes, olen );
                    goto exit;
                }

                ret = hexstring_to_binary( p, remaining_bytes,
                            tmp, &olen );
        ...
        if( mbedtls_pk_get_type( &key ) == MBEDTLS_PK_ECKEY )
        {
            ...
            if( opt.output_format == OUTPUT_FORMAT_BIN )
            {
                ...
                *p++=0x04;
                ...
                if( (olen / 2) > remaining_bytes )
                {
                    mbedtls_printf( " failed\n  !  not enough memory for Y coordinate - %d bytes left but %ld bytes needed \n\n",
                        (unsigned int) remaining_bytes, olen );
                    goto exit;
                }

                ret = hexstring_to_binary( p, remaining_bytes, tmp, &olen );
